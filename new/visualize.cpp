...
int main(int argc, char** argv) {
    ...
    GLfloat x[9] = {-0.57, -0.83, -0.47, -0.85, 0.31, 0.29, -0.29, -1.00, 0.91};
    GLfloat y[9] = { 0.19, 0.80, -0.91, 0.26, 0.92, 0.75, 0.43, -0.12, 0.62};
    GLuint vbo;
    glGenBuffers(1, &vbo);
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(x) + sizeof(y), NULL, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(x), x);
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(x), sizeof(y), y);
    GLuint vao;
    glGenVertexArrays(1, &vao);
    glBindVertexArray(vao);
    glVertexAttribPointer(0, 1, GL_FLOAT, GL_FALSE, 0, NULL);
    glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, 0, (GLvoid*)sizeof(y));
    //printf("num bytes %lu\n", numberOfBytes);
